...
def rapidfire_process(
    ...
    l_logger = get_fw_logger(
        "rocket.launcher", l_dir=l_dir, stream_level=loglvl)
    ...
    while nlaunches == 0:
        ...
        if len(live_ids) > 0:
            ...
            log_multi(
                l_logger, f"Sleeping for {sleep_time} secs before resubmit sub job")
...
def start_rockets(
    ...
    processes = [
        Process(
            ...
            args=(ifworker, nlaunches, sleep, loglvl, port, nl,
                  sub_nproc, timeout, running_ids_dict, local_redirect),
        )
        for nl, sub_nproc, ifworker in zip(node_lists, sub_nproc_list, lfworker)
    ]
...
def split_node_lists(num_jobs, total_node_list=None, ppn=24):
    ...
    if total_node_list:
        ...
        if nnodes % num_jobs != 0:
            raise ValueError(
                f"can't allocate nodes, {nnodes} can't be divided by {num_jobs}")
        sub_nnodes = nnodes // num_jobs
        sub_nproc_list = [sub_nnodes * ppn] * num_jobs
        node_lists = [orig_node_list[i: i + sub_nnodes]
                      for i in range(0, nnodes, sub_nnodes)]
    else:
...
def launch_multiprocess2(
    ...
    if exclude_current_node:
        ...
        if host in total_node_list:
            log_multi(
                l_logger, f'Remove the current node "{host}" from compute node')
            total_node_list.remove(host)
        else:
            log_multi(
                l_logger, "The current node is not in the node list, keep the node list as is")
    node_lists, sub_nproc_list = split_node_lists(
        num_jobs, total_node_list, ppn)
    ...
    ping_thread = threading.Thread(
        target=ping_multilaunch, args=(port, ping_stop))
    ping_thread.start()
