...
def optimize_inplane_scale_for_slab(
    slab0,
    calc,
    vacuum_axis=2,   # 2 means the 3rd cell vector is vacuum direction
    da=0.10,
    scan_step=0.01,
    fit_n=7,
    bounds_width=0.02,
    ...
    """
    Optimize in-plane lattice scale for a slab by scaling only the two in-plane
    lattice vectors and the corresponding Cartesian components of positions.
    """
    if vacuum_axis not in (0, 1, 2):
        raise ValueError("vacuum_axis must be 0, 1, or 2")

    in_axes = [0, 1, 2]
    in_axes.remove(vacuum_axis)

    cell0 = slab0.get_cell().array
    Lref0 = float(np.linalg.norm(cell0[in_axes[0]]))
    if Lref0 <= 0:
        raise ValueError("Bad slab cell: in-plane lattice vector length is zero")

    # scan in scale factor s (convert da in Å to scale window)
    s_min = (Lref0 - da) / Lref0
    s_max = (Lref0 + da) / Lref0
    if s_min <= 0:
        raise ValueError("da too large; leads to non-positive scale")

    s_vals = np.arange(s_min, s_max + 1e-12, scan_step / Lref0)

    def energy_at_scale(s):
        atoms = slab0.copy()

        # scale in-plane cell vectors only
        cell = atoms.get_cell().array
        cell[in_axes[0]] *= s
        cell[in_axes[1]] *= s
        atoms.set_cell(cell, scale_atoms=False)

        # scale in-plane Cartesian coordinates to match
    ...
    E_scan = np.array([energy_at_scale(s) for s in s_vals])

    nfit = min(fit_n, len(s_vals))
    inds = np.argsort(E_scan)[:nfit]
    p = np.polyfit(s_vals[inds], E_scan[inds], 2)
    if p[0] <= 0:
        raise RuntimeError("Quadratic curvature non-positive; increase da/points.")
    ...
    out = opt.minimize_scalar(
        energy_at_scale,
        method="bounded",
        bounds=(s_est - bounds_width, s_est + bounds_width),
        options={"xatol": 1e-4},
    )

    s_opt = float(out.x)
    cell_opt = cell0.copy()
    cell_opt[in_axes[0]] *= s_opt
    cell_opt[in_axes[1]] *= s_opt
    ...
    return {
        ...
        "scan_Evals_eV": E_scan.tolist(),
        "vacuum_axis": int(vacuum_axis),
        "inplane_axes": in_axes,
        "L1_inplane_opt_A": float(np.linalg.norm(cell_opt[in_axes[0]])),
        "L2_inplane_opt_A": float(np.linalg.norm(cell_opt[in_axes[1]])),
    }
...
def optimize_a_by_energy_minimization(atoms0, calc, a_guess, da=0.10, scan_step=0.01):
    ...
    Notes
    -----
    This assumes a *cubic* supercell where a single scalar 'a' is meaningful.
    For non-cubic cells from alloy_path, we scale all lattice vectors isotropically and
    report the implied conventional 'a' as:
        a = ||cell[0]|| / scx
    where scx is estimated from a_guess.
    """
    ...
    # scale scan around 1.0 using da in Angstroms interpreted in terms of 'a'
    s_vals = np.arange((a_guess - da) / a_guess, (a_guess + da) / a_guess + 1e-12, scan_step / a_guess)
...
# ============================================================
# Prep class (updated with alloy_path support)
# ============================================================
...
class Prep:
    ...
    @step(1)
    @requires("scan_inplane_slab_energy", "alloy_path")
    def slab_inplane_energy_scan(
        self,
        vacuum_axis=2,
        s_min=0.98,
        s_max=1.02,
        ...
        frac = slab0.get_scaled_positions(wrap=False)
        ...
        for s in s_vals:
            ...
            slab.calc = calc
            E_vals.append(slab.get_potential_energy())

        E_vals = np.array(E_vals)
    
        # ---- find best point ----
        i0 = int(np.argmin(E_vals))
        s_best = float(s_vals[i0])
        E_best = float(E_vals[i0])
    
        # ---- compute in-plane "lattice constants" at best scale ----
        cell0 = slab0.get_cell().array
        in_axes = [0, 1, 2]
        in_axes.remove(vacuum_axis)
    
        L1_0 = float(np.linalg.norm(cell0[in_axes[0]]))
        L2_0 = float(np.linalg.norm(cell0[in_axes[1]]))
    
        L1_best = s_best * L1_0
        L2_best = s_best * L2_0
        a2d_best = 0.5 * (L1_best + L2_best)
    
        # optional: for fcc(111), convert 2D lattice constant to equivalent cubic a0
        a0_equiv_fcc = float(np.sqrt(2.0) * a2d_best)
    
        # store on self so runprep.py can print after run_selected()
        self.inplane_scale = s_best
        self.L1_inplane_A = L1_best
        self.L2_inplane_A = L2_best
        self.a2d_inplane_A = a2d_best
        self.a0_equiv_fcc_A = a0_equiv_fcc
    
        best = {
            "scale": s_best,
            "energy_eV": E_best,
            "L1_inplane_A": L1_best,
            "L2_inplane_A": L2_best,
            "a2d_inplane_A": a2d_best,
            "a0_equiv_fcc_A": a0_equiv_fcc,
        }
    
        # ---- record provenance outputs (THIS is the "how to add them as output") ----
        self._record_step(
            "slab_inplane_energy_scan",
            inputs={
                "alloy_path": self.alloy_path,
            ...
            outputs={
                "s_vals": [float(x) for x in s_vals],
                "E_vals_eV": [float(x) for x in E_vals],
                "best": best,
            },
        )
    
        return s_vals, E_vals
...
#                "scale_opt": res.get("scale_opt"),
#                "scx_est": res.get("scx_est"),
#                "scan_scales": res.get("scan_scales"),
...
#        return self.a
    ...
    def generate_slab(self, a=None):
        ...
        if self.software != "XTB":
            ...
            # You likely have this in your original codebase; not provided in the snippets.
